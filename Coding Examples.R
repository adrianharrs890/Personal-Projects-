...
# Covid Research at NYU, Kaggle Comps, and various data related projects 
# The more elaborate that ive written/co worked on will have to be should in person
# Due to the code being confidential and etc 
...
for(i in 1:iter){
  ...
  testVec[[i]]  <- confusionMatrix(predict(mod, test), var)$overall["Accuaracy"]
  for(j in 1:length()){
    rslt[[j]][i] <- as.numeric(imp[[i]] %>% filter(rownames == varNames[j])) %>% select(-rownames)
  }
}


# Then you can unlist these values, compute means, SE, and etc of variable imp.

imp %>%
  gather(Variable, value, -c(varNames)) %>%
  group_by(Variable) %>%
  top_n(50, value) %>%
  arrange(desc(value))
